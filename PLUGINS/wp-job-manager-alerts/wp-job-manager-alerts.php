...
class WP_Job_Manager_Alerts {
	 ...
	 * Post types class.
	 *
	 * @var WP_Job_Manager_Alerts_Post_Types
	 */
	public WP_Job_Manager_Alerts_Post_Types $post_types;
	...
	public function __construct() {
		// Define constants
		define( 'JOB_MANAGER_ALERTS_VERSION', '2.0.0' );
	...
	public function init_plugin() {
		...
		include 'includes/class-wp-job-manager-alerts-admin.php';

		// Init classes
		$this->post_types = new WP_Job_Manager_Alerts_Post_Types();

		new WP_Job_Manager_Alerts_Admin();
...
Unsubscribe from this alert through the link: {alert_unsubscribe_url}',
	...
	public function alert_link( $links, $args ) {
		if ( is_user_logged_in() && get_option( 'job_manager_alerts_page_id' ) ) {
			...
			$job_type_array = [];
			foreach ( $args['filter_job_types'] as $job_type ) {
